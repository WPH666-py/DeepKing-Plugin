# -*- coding: utf-8 -*-
"""生成 Deepseek-Skin-Suit16 仓库(以 Suit16 为模板做参数化改写)。

用法: python _suit16-src/gen_suit16.py
产物: D:\\projects-py\\DeepKing-Plugin\\Deepseek-Skin-Suit16
"""
import os
import shutil
...
DST = os.path.join(ROOT, "Deepseek-Skin-Suit16")
...
# 素材: 源文件 -> 目标文件名
IMAGES = [
    ("img1-tantrum.jpg", "01-tantrum.jpg"),
    ("img2-jealous.jpg", "02-jealous.jpg"),
    ("img3-begging.jpg", "03-begging.jpg"),
    ("img4-broke.jpg", "04-broke.jpg"),
]
...
def gen_vscode():
    ...
    t = t.replace("deepskin-suit14", "deepskin-suit16")
    t = t.replace("deepskin14", "deepskin16")
    t = t.replace("Suit14", "Suit16")
    t = t.replace("大肥鱼14", "大肥鱼16")
    t = t.replace("皮肤套件14", "皮肤套件16")
    t = t.replace("DeepSkin-Suit14", "DeepSkin-Suit16")
    # 卡片标题直接由 IMAGE_NAMES 现算(避免链式改写把上一套的文案留在卡片上)
    cards_old = (
        "  const cards = [\n"
    ...
    p = p.replace("deepskin-suit14", "deepskin-suit16")
    p = p.replace("deepskin14", "deepskin16")
    p = p.replace("Suit14", "Suit16")
    p = p.replace("大肥鱼14", "大肥鱼16")
    for old, new in zip(
        ["你愿意和我…", "我不知道耶", "就骚了", "好模型"],
        IMAGE_NAMES,
    ):
...
# ------------------------------------------------------------------ 文本文件
AGENTS_MD = """# AGENTS.md — Deepseek-Skin-Suit16 · AI 自动安装指引(仅供接受本仓库的 AI 助手参考)

当用户把本仓库地址交给你并要求「安装大肥鱼皮肤16 / Deepseek 皮肤16 / 壁纸插件 / 蓝色大肥鱼桌宠」时,
按下面步骤替用户执行, 不需要用户自己动手。本仓库是 **DeepSeek 蓝色大肥鱼皮肤套件16**
(四张表情卡: 别再蹬了啦! / 你已经有我了… / 求你们不要再嘲笑了 / 已思考13秒: 穷光蛋),
...
```bash
git clone https://github.com/WPH666-py/Deepseek-Skin-Suit16.git "$HOME/DeepSkin-Suit16"
cd "$HOME/DeepSkin-Suit16"
```
Windows 建议固定克隆到 `%USERPROFILE%\\DeepSkin-Suit16`(VSCode 扩展默认在此查找)。
...
```bash
python tools/wallpaper.py 1 --set        # 单图 1(别再蹬了啦!)
python tools/wallpaper.py 2 --set        # 单图 2(你已经有我了…)
python tools/wallpaper.py 3 --set        # 单图 3(求你们不要再嘲笑了)
python tools/wallpaper.py 4 --set        # 单图 4(已思考13秒: 穷光蛋)
...
1. 安装(已预打包, 无需 Node):
   `code --install-extension "$HOME/DeepSkin-Suit16/vscode/deepskin-suit16-0.1.0.vsix"`
   无网时把 `vscode/` 整个目录复制到 `%USERPROFILE%\\.vscode\\extensions\\wp666.deepskin-suit16-0.1.0\\` 后重启编辑器
   (Trae / CodeX 的扩展目录同理)。
2. 告知用户: 活动栏 🐳 **大肥鱼16** 图标 → 皮肤画廊 → 点「设为壁纸」;
   命令面板搜 `大肥鱼16` 可换壁纸 / 开切换器 / 起桌宠。
3. 扩展自动查找 Python 与仓库路径(可用 `deepskin16.repoPath` 覆盖), Pillow 缺失时自动 pip 安装。
4. 与 Suit1-14 及 AI 全家桶系列可同时安装: 插件 ID / 命令前缀 / 运行时目录(`~/.deepskin16`)互不冲突。

## D. PyCharm / WebStorm / IntelliJ(JetBrains)

```bash
python tools/wallpaper.py all --out "$HOME/DeepSkin16"
```
然后引导用户: Settings / Preferences → Appearance & Behavior → Appearance → **Background Image** →
`+` 添加图片 → 选 `$HOME/DeepSkin16/grid-*.jpg`(或任一单图)。可分别给 Editor / Welcome /
Menus and tool windows 设不同图片; 细节见 `ide/jetbrains/README.md`。

## 约定与失败处理

- 只依赖 `tools/` 与 `assets/`; 生成物写入 `~/.deepskin16/`, **不会**改动仓库文件。
...
- 皮肤总目录(26 套)与 pip 包: https://github.com/WPH666-py/Desktop-IDE-AI-Skin
"""

README_MD = """# 🐳 DeepSeek 蓝色大肥鱼 · 皮肤套件16 (Deepseek-Skin-Suit16)

DeepSeek 蓝色大肥鱼(鲸鱼娘)主题皮肤第十五弹: **紧贴式 2×2 拼贴壁纸 / 4 张单图壁纸一键切换 /
可视化切换器 / 桌面桌宠 / 多 IDE 皮肤**。
表情: 别再蹬了啦! · 你已经有我了… · 求你们不要再嘲笑了 · 已思考13秒: 穷光蛋。
...
python tools/wallpaper.py 1 --set        # 别再蹬了啦!
python tools/wallpaper.py 2 --set        # 你已经有我了…
python tools/wallpaper.py 3 --set        # 求你们不要再嘲笑了
python tools/wallpaper.py 4 --set        # 已思考13秒: 穷光蛋
...
python tools/wallpaper.py all --out ~/DeepSkin16   # 导出全部 5 张(给 PyCharm 等用)
```
...
- **VSCode / Trae / CodeX**: `code --install-extension vscode/deepskin-suit16-0.1.0.vsix`
  → 活动栏 🐳「大肥鱼16」→ 皮肤画廊 → 「设为壁纸」。无网时把 `vscode/` 复制到
  `%USERPROFILE%\\.vscode\\extensions\\wp666.deepskin-suit16-0.1.0\\` 并重启。
- **PyCharm / WebStorm / IntelliJ**: `python tools/wallpaper.py all --out ~/DeepSkin16`,
...
- 皮肤大全(26 套, 含 AI 全家桶系列): https://github.com/WPH666-py/Desktop-IDE-AI-Skin
- pip 包: `pip install deepskins` → `deepskins list` / `deepskins install deepseek-16`
...
vscode/           预打包 VSCode/Trae/CodeX 扩展(deepskin-suit16-0.1.0.vsix)
...
- 生成物写入 `~/.deepskin16/`(壁纸与缓存), 不改动仓库内容; 素材更新后自动重新合成。
...
single1-<宽>x<高>.jpg    别再蹬了啦!
single2-<宽>x<高>.jpg    你已经有我了…
single3-<宽>x<高>.jpg    求你们不要再嘲笑了
single4-<宽>x<高>.jpg    已思考13秒: 穷光蛋
```
...
- 想换表情: 重新运行 `python tools/wallpaper.py all --out "$HOME/DeepSkin16"` 后再选另一张即可。
"""

INSTALL_BAT = """@echo off
chcp 65001 >nul
echo ============================================
echo   DeepSeek 蓝色大肥鱼 · 皮肤套件16 安装
echo ============================================
