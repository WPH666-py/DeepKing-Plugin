# -*- coding: utf-8 -*-
"""生成 Deepseek-Skin-Suit19 仓库。

模板: _suit19-src/tpl/(取自 Suit12 的"单张样式"套件)
特点: IMAGE_FILES 只有 1 张素材 -> grid = 全屏单图(cover), single1 = 卡片单图。
脚本与文档都按 Suit19 显式写出(不再靠对上一套做链式替换, 避免文案串味)。

用法: python _suit19-src/gen_suit19.py
产物: D:\\projects-py\\DeepKing-Plugin\\Deepseek-Skin-Suit19
"""
import os
import shutil
...
SRC = HERE
DST = os.path.join(ROOT, "Deepseek-Skin-Suit19")

NUM = "19"
ASSET_SRC = "img1-charge.jpg"        # 素材源(充值通道插画, 1080x1095)
ASSET_DST = "01-charge.jpg"          # 仓库内文件名
ASSET_STEM = "01-charge"
WALLPAPER_NAME = "特别充值通道 1鲸子=1tonken"
PET_NAME = "鲸鱼娘"
# 桌宠: 取人物区域(方形取景, 角色居中)
PET_CROP = (0.70, 0.50, 0.32)        # (中心x比例, 中心y比例, 半边长占最短边比例)
BG_TOL = 26
BG_BRIGHT = 168
...
def compose_fit(size=(1920, 1080)):
    ...
    方形/竖版素材在 16:9 屏幕上 cover 会裁掉约 22% 的高度, 文案会被切;
    这个模式保证整张素材都在画面内。
    """
    ensure_pillow()
...
(单张插画: 鲸鱼娘提裙比嘘, 配字「这是特别充值通道, 1鲸子=1tonken(仅限中出)」),
...
**关于裁切**: 素材接近 1:1 方形, 16:9 屏上用 cover 铺满会裁掉上下各约 22%, 大字文案会被切;
...
DeepSeek 蓝色大肥鱼(鲸鱼娘)主题皮肤第十七弹: **单张样式**——一张充值通道插画,
提供 **完整卡片单图**(整张不裁切, 默认)与 **全屏铺满单图** 两种壁纸形态,
外加可视化切换器、桌面桌宠与多 IDE 皮肤。
插画: 鲸鱼娘提裙比「嘘」, 大字「这是特别充值通道, 1鲸子=1tonken」, 小字「(仅限中出)」。
...
> 素材接近 1:1 方形。默认的「完整卡片单图」保证大字与画中人完整可见;
...
```
assets/           1 张插画(01-charge.jpg, 1080x1095)
