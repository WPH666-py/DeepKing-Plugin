...
PAIRS = [
    ...
    ('DST = os.path.join(ROOT, "Deepseek-Skin-Suit17")',
     'DST = os.path.join(ROOT, "Deepseek-Skin-Suit21")', 1),
    ('NUM = "17"', 'NUM = "21"', 1),

    # ---------------------------------------------------------------- 素材参数
    ('ASSET_SRC = "img1-quad.jpg"          # 素材源(四格拼图, 960x960)',
     'ASSET_SRC = "img1-ginkgo.jpg"        # 素材源(银杏银饰特写, 1104x637 = 16:9)', 1),
    ('ASSET_DST = "01-quad.jpg"            # 仓库内文件名',
     'ASSET_DST = "01-ginkgo.jpg"          # 仓库内文件名', 1),
    ('ASSET_STEM = "01-quad"', 'ASSET_STEM = "01-ginkgo"', 1),
    ('WALLPAPER_NAME = "鲸鱼四连(2×2 四格)"',
     'WALLPAPER_NAME = "银杏银饰(16:9 宽幅)"', 1),
    ...
    ('PET_CROP = (0.26, 0.26, 0.18)        # (中心x比例, 中心y比例, 半边长占最短边比例)',
     'PET_CROP = (0.47, 0.50, 0.44)        # (中心x比例, 中心y比例, 半边长占最短边比例)', 1),
    ...
    ("DeepSeek 蓝色大肥鱼(鲸鱼娘)主题皮肤第十七弹: **单张样式**——一张四格表情拼图,",
     "DeepSeek 蓝色大肥鱼(鲸鱼娘)主题皮肤第二十一弹: **单张样式**——一张银杏银饰特写,", 1),
    ...
    ("assets/           1 张四格表情拼图(01-quad.jpg, 960x960)",
     "assets/           1 张特写插画(01-ginkgo.jpg, 1104x637)", 1),
...
def swap_modes_lines(text):
    ...
    if len(hit) != 1:
        raise SystemExit("[make21] MODES 定位失败, 命中 %d 行" % len(hit))
    a = hit[0]
    b = a + 1
    if "全屏单图(铺满, 上下裁切)" not in lines[b]:
        raise SystemExit("[make21] MODES 第二行不是预期内容")
    lines[a] = lines[a].replace("完整单图(默认, 不裁切)", "全屏铺满(默认, cover)")
    lines[b] = lines[b].replace("全屏单图(铺满, 上下裁切)", "卡片单图(模糊填充)")
    print("[make21] MODES 行已换: 第 %d-%d 行" % (a + 1, b + 1))
    return "\n".join(lines)
...
def main():
    ...
    for old, new, cnt in PAIRS:
        n = text.count(old)
        if n == 0:
            raise SystemExit("[make21] 未找到片段(0 次):\n%r" % old[:200])
        if n != cnt:
            raise SystemExit("[make21] 片段出现 %d 次, 期望 %d:\n%r" % (n, cnt, old[:200]))
    ...
    with open(DST, "w", encoding="utf-8", newline="\n") as f:
        f.write(text)
    print("[make21] 写出 %s (%d bytes), %d 条替换 + MODES 换行"
    ...
    for label, keys in (("17", ("17",)),
        ...
        if bad:
            print("[make21] 仍含 %s 的行:" % label)
            for i, l in bad:
                print("   %4d  %s" % (i, l))
        else:
            print("[make21] 自检通过: 无 %s 残留" % label)
