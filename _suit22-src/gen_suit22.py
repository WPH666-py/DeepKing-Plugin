# -*- coding: utf-8 -*-
"""生成 Deepseek-Skin-Suit22 仓库。

模板: _suit20-src/tpl/(取自 Suit12 的"单张样式"套件)
特点: IMAGE_FILES 只有 1 张素材 -> grid = 全屏单图(cover), single1 = 卡片单图。
脚本与文档都按 Suit22 显式写出(不再靠对上一套做链式替换, 避免文案串味)。

用法: python _suit22-src/gen_suit22.py
...
SRC = HERE
DST = os.path.join(ROOT, "Deepseek-Skin-Suit22")

NUM = "22"
ASSET_SRC = "img1-drum.jpg"          # 素材源(苗寨鼓楼插画, 1104x629 = 16:9)
ASSET_DST = "01-drum.jpg"            # 仓库内文件名
ASSET_STEM = "01-drum"
WALLPAPER_NAME = "苗寨鼓楼(16:9 宽幅)"
PET_NAME = "青羽"
# 桌宠: 取人物区域(方形取景; 特写插画无白底可抠)
PET_CROP = (0.42, 0.66, 0.44)        # (中心x比例, 中心y比例, 半边长占最短边比例)
BG_TOL = 26
BG_BRIGHT = 168
...
| 功能 | 说明 |
|---|---|
| 全屏铺满单图 | 素材按 cover 铺满整屏(默认; 1104x629 与屏幕同形, 几乎不裁切) |
